...
if PINECONE_API_KEY and PineconeVectorStore is not None and OpenAIEmbeddings is not None and EMBEDDINGS_MODEL_SETTING == "OPENAI":
    print("🤖 Mode: Connecting to Cloud OpenAI Index (1536 Dim Space)...")
    embeddings = OpenAIEmbeddings()
    INDEX_NAME = "fde-sop-index-openai"
elif PINECONE_API_KEY and PineconeVectorStore is not None:
...
if PINECONE_API_KEY and PineconeVectorStore is not None:
...
def query_telemetry_db(sql_query: str) -> str:
    ...
    if not db_password:
        ...
        csv_path = project_root / "data" / "raw" / "dynamic_supply_chain_logistics_dataset.csv"
        frame = pd.read_csv(csv_path).head(10)
        return "LOCAL DEMO TELEMETRY (SQL Server credentials not configured)\n" + frame.to_string(index=False)
...
def fetch_corridor_conditions(latitude: float, longitude: float) -> str:
    ...
    try:
        ...
        return (
            f"LOCAL CORRIDOR FALLBACK\nTarget GPS: {latitude}, {longitude}\n"
            "External conditions unavailable; verify live conditions before dispatch."
        )
...
def search_compliance_sop(query: str) -> str:
    ...
    try:
        if retriever is None:
            sop_path = project_root / "data" / "policy" / "Cold_Chain_Incident_SOP_v2.md"
            sop_text = sop_path.read_text(encoding="utf-8")
            terms = [term.lower() for term in query.split() if len(term) > 3]
            sections = sop_text.split("\n## ")
            matches = [section for section in sections if any(term in section.lower() for term in terms)]
            return "--- LOCAL SOP CONTEXT ---\n" + "\n\n".join(matches or sections[:2]) + "\n-------------------------"
